    ...
    useEffect(() => {
        const fetchMatiereCoursEtFichiers = async () => {
            try {
                ...
                // Appel API : Assurez-vous que ces endpoints dans Django 
                // utilisent IsAuthenticated et filtrent par request.user
                const [matiereResponse, coursResponse, fichiersResponse] = await Promise.all([
                    axiosInstance.get(`/matieres/${id}/`),
                    axiosInstance.get(`/cours/?matiere=${id}`),
                    axiosInstance.get(`/fichiers-cours/`)
                ]);

                setMatiereInfo(matiereResponse.data);

                // Filtrage strict côté client par sécurité supplémentaire
                ...
                // Si l'API renvoie 403 ou 404 car l'enseignant n'a pas accès, on gère l'erreur
                setError("Vous n'avez pas accès à ces données ou elles n'existent pas.");
            } finally {
                setLoading(false);
            }
        };
